...
public enum CalculateOperatorEnum {
    ...
    AND("and", "IF [expr1] and [expr3]", "对两个表达式进行逻辑或操作", "if ([${name}='张三'] and [${age}>25]) then ['结果1'] else ['结果2'] end"),
    OR("or", "IF [expr1] or [expr2] ", "对两个表达式进行逻辑合取操作", "if ([${name}='张三'] or [${age}>25]) then ['结果1'] else ['结果2'] end"),
