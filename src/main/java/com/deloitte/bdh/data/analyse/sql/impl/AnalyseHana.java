...
import com.deloitte.bdh.data.analyse.sql.utils.HanaBuildUtil;
...
@Slf4j
@Service("analysehana")
public class AnalyseHana extends AbstractRela {
    @Resource
...
//        String limit = this.page(context);
    ...
    protected String from(DataModel model) {
        return " FROM " + HanaBuildUtil.from(model.getTableName(), null);
    }
    ...
    protected String where(DataModel model) {
        ...
        list.add(" 1=1 ");
        for (DataModelField s : model.getX()) {
            String express = HanaBuildUtil.where(model.getTableName(), s.getId(), s.getQuota(), s.getSymbol(), s.getValue());
            if (StringUtils.isNotBlank(express)) {
                list.add(express);
            }
        }
        if (CollectionUtils.isNotEmpty(model.getConditions())) {
            for (DataCondition condition : model.getConditions()) {
                ...
                express = HanaBuildUtil.where(model.getTableName(), condition.getId().get(0), condition.getQuota(), condition.getFormatType(), symbol, value);
                list.add(express);
            }
        }
        //权限条件
        if (StringUtils.isNotBlank(model.getPageId())) {
            ...
            if (CollectionUtils.isNotEmpty(userDataList)) {
                for (BiUiAnalyseUserData userData : userDataList) {
                    ...
                    String express = HanaBuildUtil.where(userData.getTableName(), userData.getTableField(), DataModelTypeEnum.WD.getCode(), WildcardEnum.EQ.getCode(), value);
                    if (StringUtils.isNotBlank(express)) {
                        list.add(express);
                    }
                }
    ...
    protected String groupBy(DataModel model) {
        ...
        if (CollectionUtils.isNotEmpty(model.getX())) {
            ...
            for (DataModelField s : model.getX()) {
                String express = HanaBuildUtil.groupBy(model.getTableName(), s.getId(), s.getQuota()
    ...
    protected String having(DataModel model) {
        List<String> list = Lists.newArrayList();
        for (DataModelField s : model.getX()) {
            String express = HanaBuildUtil.having(model.getTableName(), s.getId(), s.getQuota()
        ...
        if (CollectionUtils.isNotEmpty(model.getConditions())) {
            for (DataCondition condition : model.getConditions()) {
                ...
                if (StringUtils.equals(condition.getQuota(), DataModelTypeEnum.DL.getCode()) &&
                    ...
                    String express = HanaBuildUtil.having(model.getTableName(), condition.getId().get(0), condition.getQuota(),
    ...
    protected String orderBy(DataModel model) {
        List<String> list = Lists.newArrayList();
        for (DataModelField s : model.getX()) {
            String express = HanaBuildUtil.orderBy(model.getTableName(), s.getId(), s.getQuota()
    ...
    protected String page(SqlContext context) {
        ...
        return " LIMIT " + model.getPageSize() + " offset " + (model.getPage() - 1) * model.getPageSize();
    }
